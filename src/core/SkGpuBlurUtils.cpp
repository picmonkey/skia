...
static void convolve_gaussian_1d(GrRenderTargetContext* renderTargetContext,
    ...
    sk_sp<GrFragmentProcessor> conv(GrGaussianConvolutionFragmentProcessor::Make( resourceProvider, std::move(proxy), direction, radius, sigma, mode == GrTextureDomain::kIgnore_Mode, bounds));
...
static GrPixelConfig get_blur_config(GrTextureProxy* proxy, SkColorSpace* cs) {
    ...
    SkASSERT(kBGRA_8888_GrPixelConfig == config || kRGBA_8888_GrPixelConfig == config ||
             kRGBA_4444_GrPixelConfig == config ||
...
static sk_sp<GrRenderTargetContext> convolve_gaussian_2d(GrContext* context,
    ...
    renderTargetContext = context->makeDeferredRenderTargetContext(
    ...
    GrResourceProvider* resourceProvider = renderTargetContext->resourceProvider();
    auto conv = GrMatrixConvolutionEffect::MakeGaussian(resourceProvider, std::move(proxy), srcBounds, size, 1.0, 0.0,
...
static sk_sp<GrRenderTargetContext> convolve_gaussian(GrContext* context,
    ...
    if (Gr1DKernelEffect::kX_Direction == direction) {
    ...
    if (!topRect.isEmpty()) {
        dstRenderTargetContext->clear(&topRect, 0, false);
    }
    
    if (!bottomRect.isEmpty()) {
        dstRenderTargetContext->clear(&bottomRect, 0, false);
    }
...
static sk_sp<GrTextureProxy> decimate(GrContext* context,
    ...
    for (int i = 1; i < scaleFactorX || i < scaleFactorY; i *= 2) {
        ...
        if (GrTextureDomain::kIgnore_Mode != mode && i == 1) {
            ...
            auto fp = GrTextureDomainEffect::Make(context->resourceProvider(),
                                                  std::move(src),
                                                  nullptr,
                                                  SkMatrix::I(),
                                                  domain,
            ...
            paint.addColorTextureProcessor(context->resourceProvider(), std::move(src),
...
static sk_sp<GrRenderTargetContext> reexpand(GrContext* context,
    ...
    sk_sp<GrRenderTargetContext> dstRenderTargetContext =
    context->makeDeferredRenderTargetContext(fit, dstII.width(), dstII.height(),
                                                           config, dstII.refColorSpace());
    if (!dstRenderTargetContext) {
        return nullptr;
    }
    ...
    if (GrTextureDomain::kIgnore_Mode != mode) {
        ...
        GrSamplerParams params(SkShader::kClamp_TileMode, GrSamplerParams::kBilerp_FilterMode);
        paint.addColorTextureProcessor(context->resourceProvider(), std::move(srcProxy), nullptr, SkMatrix::I(),
                                       params);
    }
    paint.setPorterDuffXPFactory(SkBlendMode::kSrc);
    GrFixedClip clip(dstII.bounds());
    ...
    sk_sp<GrRenderTargetContext> GaussianBlur(GrContext* context,
        ...
        if (sigmaX > 0.0f) {
            dstRenderTargetContext = convolve_gaussian(context, std::move(srcProxy), srcRect, srcOffset,
                                                       Direction::kX_Direction, radiusX, sigmaX, &localSrcBounds,
        ...
        if (sigmaY > 0.0f) {
            dstRenderTargetContext = convolve_gaussian(context, std::move(srcProxy), srcRect, srcOffset,
                                                       Direction::kY_Direction, radiusY, sigmaY, &localSrcBounds,
